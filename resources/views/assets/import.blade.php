...
<div class="d-flex justify-content-center py-4">
    <div class="card shadow-sm border-0 rounded-4 w-100" style="max-width: 950px;">
        ...
        <div class="card-body p-4" style="background-color: #fafafa;">
            ...
            {{-- Botón volver --}}
            <div class="mb-3">
                <a href="{{ route('assets.index') }}" class="btn btn-secondary px-4 py-2">
                    <i class="fas fa-arrow-left me-1"></i> Volver al listado
                </a>
            </div>

            {{-- Instrucciones --}}
            <p>Para importar activos correctamente, descarga la plantilla y completa los campos requeridos. Solo se
                aceptan archivos CSV o Excel (.csv, .xls, .xlsx). Sigue las instrucciones detalladas para evitar errores
                durante la importación.
                <li>
                    Debes <strong>registrar previamente a los empleados</strong> y
                    <strong>seleccionar al encargado de la Unidad de Informática</strong>.
                </li>
            </p>
            <div class="mb-4 d-flex gap-2 flex-wrap">
                <a href="{{ route('assets.template.download') }}" class="btn btn-guinda">
                    <i class="fas fa-download me-1"></i> Descargar Plantilla
                </a>
                <a href="{{ route('assets.instructions.pdf') }}" target="_blank" class="btn btn-secondary">
            ...
            {{-- Formulario --}}
            <form action="{{ route('assets.import') }}" method="POST" enctype="multipart/form-data" id="importForm">
                @csrf
                <div class="row g-3">
                    ...
                    <div class="col-12 mb-3" id="importSummary" style="display:none;">
                        ...
                        <div class="d-flex justify-content-end mt-2">
                            <button type="button" id="confirmImport" class="btn btn-guinda me-2" style="display:none;"
                                data-confirm-import
                                data-text="Se importarán únicamente los activos válidos. ¿Deseas continuar?"
                                data-on-confirm="submitAssetsImport">
...
<script>
    $(document).ready(function () {
        ...
        const requiredColumns = ['tag', 'equipo', 'marca', 'modelo', 'serie', 'estado', 'propiedad', 'proveedor', 'unidad', 'departamento', 'resguardo', 'activo'];
        ...
        function previewFile(file) {
            ...
            reader.onload = function (e) {
                ...
                let seriesSet = new Set(), duplicateSeries = new Set();

                dataRows.forEach(row => {
                    const rowData = {};
                    headers.forEach((h, i) => rowData[h] = row[i] ? row[i].toString().trim() : '');
                    if (rowData['serie']) {
                        if (seriesSet.has(rowData['serie'])) duplicateSeries.add(rowData['serie']);
                        else seriesSet.add(rowData['serie']);
                    }
                });
                ...
                dataRows.forEach((row, index) => {
                    ...
                    if (!rowData['tag'] && !rowData['serie']) issues.push('Sin TAG y sin Serie, no se puede importar');
                    if (rowData['serie'] && duplicateSeries.has(rowData['serie'])) issues.push('Serie duplicada, no se puede importar');

                    if (issues.length) rowIssues.push({ row: index + 2, issues, rowData });
                ...
                showMessage('Archivo revisado. Revisa el resumen antes de importar.', 'info');
            };
        ...
        window.submitAssetsImport = function () {
            localStorage.setItem('import_status', 'active'); // Despertar poller en otras pestañas
            document.getElementById('importForm').submit();
        };

        $('#cancelImport').on('click', function () { $('#file').val(''); $('#previewContainer').hide(); $('#importSummary').hide(); showMessage('Importación cancelada.', 'info'); });
