...
@section('title', 'Exportar Activos')
    ...
    <div class="card border-0 shadow-soft rounded-4 w-100" style="max-width: 1000px;">
        ...
        <div class="card-header bg-guinda text-white rounded-top-4 py-3 px-4">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h5 class="mb-0 fw-bold">Exportar Activos</h5>
        ...
        <div class="card-body p-4 bg-light-soft">
            ...
            <form action="{{ route('assets.export') }}" method="POST" id="exportForm" target="_blank">
                @csrf
                        ...
                        <div class="col-md-6" id="filter_value_container">
                            <div class="input-group input-group-lg">
                                ...
                                <select name="filter_value" id="filter_value" class="form-select" disabled>
                                    <option value="">-- Selecciona un filtro primero --</option>
                                </select>
                            </div>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    exportForm.addEventListener('submit', function(e) {
        const col = document.getElementById('filter_column')?.value;
        const val = document.getElementById('filter_value')?.value;

        if (col && (!val || val === '')) {
            alert('Debes seleccionar un valor para el filtro.');
            e.preventDefault();
            return false;
        }
    ...
    filterColumn.addEventListener('change', function() {
        ...
        const renderSelect = (icon, placeholder, optionsHtml, disabled = false) => {
            filterContainer.innerHTML = `
            <div class="input-group input-group-lg">
                <span class="input-group-text bg-light text-muted">${icon}</span>
                <select name="filter_value" id="filter_value" class="form-select" ${disabled ? 'disabled' : 'required'}>
        ...
        // Si no hay filtro elegido
        if (!col) {
            renderSelect('<i class="fas fa-sliders-h"></i>', '-- Selecciona un filtro primero --', '', true);
            return;
        }
        ...
        if (col === 'department_id') {
            ...
            renderSelect('<i class="fas fa-building"></i>', '-- Selecciona Departamento --', opts);
            return;
        }
        ...
        if (col === 'resguardo') {
            ...
            renderSelect('<i class="fas fa-user"></i>', '-- Selecciona Resguardante --', opts);
            return;
        }

        // -------- TIPO DE EQUIPO --------
        if (col === 'device_type_equipo') {
            let opts = '';
            tipos.forEach(tipo => {
                opts += `<option value="${tipo}">${tipo}</option>`;
            });
            renderSelect('<i class="fas fa-desktop"></i>', '-- Selecciona Tipo de Equipo --', opts);
            return;
        }

        // -------- ESTADO --------
        if (col === 'estado') {
            const estados = ['OPERACION','RESGUARDADO','DANADO','BAJA','SINIESTRO','GARANTIA','OTRO'];
            let opts = estados.map(e => `<option value="${e}">${e}</option>`).join('');
            renderSelect('<i class="fas fa-info-circle"></i>', '-- Selecciona Estado --', opts);
            return;
        }

        // -------- SI CAE AQUÍ, NO SE PERMITE FILTRAR --------
        renderSelect('<i class="fas fa-ban"></i>', 'Este filtro no está disponible por selección', '', true);
    });
