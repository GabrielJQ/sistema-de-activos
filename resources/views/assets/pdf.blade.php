...
<head>
    <meta charset="UTF-8">
    <title>{{ 'relacionActivos_' . now()->format('Y-m-d_H-i-s') }}</title>
    <style>
        /* Configuración general */
        body {
            font-family: Arial, sans-serif;
            font-size: 10px; /* tamaño reducido para muchas columnas */
            margin: 10px;
        }
        ...
        .data-table th, .data-table td {
            ...
            max-width: 120px;        /* ancho máximo por celda */
        }
        .data-table th {
...
<!-- Título del reporte -->
<div class="title">RELACIÓN DE ACTIVOS</div>
        ...
        @foreach($data as $row)
            <tr>
                @foreach($row as $key => $value)
                    <td class="{{ in_array($key, ['ColumnasExtras']) ? 'extra-column' : '' }}">
                        {{ $value }}
                    </td>
                @endforeach
            </tr>
